            ...
            child: BlocBuilder<ProfileBloc, ProfileState>(
              builder: (context, state) {
                if (state is ProfileLoading) {
                  ...
                  return profileView(state.employee, context);
                } else if (state is ProfileError) {
                  return Center(child: Text(state.error));
                }
                return const SizedBox.shrink();
  ...
  Widget profileView(EmployeeModel emp, BuildContext context) {
    Widget row(String title, String? value) {
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          getCustomFont(title, 16, textColor, 1, fontWeight: FontWeight.w400),
          getVerSpace(FetchPixels.getPixelHeight(6)),
          getCustomFont(value ?? '-', 16, Colors.black, 1, fontWeight: FontWeight.w500),
          getVerSpace(FetchPixels.getPixelHeight(20)),
          getDivider(dividerColor, 0, 1),
          getVerSpace(FetchPixels.getPixelHeight(20)),
        ],
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          row("Name", emp.name),
          row("Email", emp.email),
          row("Primary Phone", emp.phone1),
          row("Secondary Phone", emp.phone2),
          row("Street", emp.street),
          row("City", emp.city),
          row("State", emp.state),
          row("Country", emp.country),
          row("Postal Code", emp.postalCode),
          row("Building Name", emp.buildingName),
          row("House Number", emp.houseNumber),
          row("Apartment Number", emp.apartmentNumber),
          row("Passport Number", emp.passportNumber),
          row("Contract Type", emp.contractType),
          row("Contract Validity ", emp.contractValidity  ),
          row("Emirates Id", emp.emiratesId ),
          row("Emirates Id", emp.emiratesIdExpiryDate  ),
          row("Visa Number ", emp.visaNumber ),
          row("Visa Expiry Date", emp.visaExpiryDate  ),
