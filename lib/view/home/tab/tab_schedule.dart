...
//                 context.read<SubscriptionScheduleBloc>().add(LoadAssignments(date: formatted));
//               }
//             },
//           ),
//         ],
...
class _TabScheduleState extends State<TabSchedule> {
  ...
  void _loadAssignments() {
    final formattedDate = DateFormat('yyyy-MM-dd').format(selectedDate ?? DateTime.now());
    context.read<SubscriptionScheduleBloc>().add(LoadAssignments(date: formattedDate));
  }
    ...
    return SafeArea(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            getVerSpace(FetchPixels.getPixelHeight(16)),
            getCustomFont("Schedule Assignments", 22, Colors.black, 1, fontWeight: FontWeight.bold),
            getVerSpace(FetchPixels.getPixelHeight(16)),
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
              ...
              child: BlocBuilder<SubscriptionScheduleBloc, SubscriptionScheduleState>(
                builder: (context, state) {
                  if (state is ScheduleLoading) {
                    ...
                    final assignments = state.assignments;
                    if (assignments.isEmpty) {
                      return Center(child: getCustomFont("No assignments found", 16, Colors.grey, 1));
                    }
                    ...
                    return ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        final a = assignments[index];
                        final workDate = DateFormat('dd MMM yyyy').format(DateTime.parse(a.workDate));
                        final workTime = a.time != null ? DateFormat('hh:mm a').format(DateTime.parse(a.time!)) : '-';
                        ...
                        return Container(
                          ...
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              getCustomFont("Customer: ${a.customer.name}", 16, Colors.black, 1, fontWeight: FontWeight.w600),
                              getVerSpace(6),
                              getCustomFont("Date: $workDate (${a.dayOfWeek})", 14, textColor, 1),
                              getCustomFont("Time: $workTime", 14, textColor, 1),
                              getCustomFont("Schedule ID: ${a.scheduleId}", 14, textColor, 1),
                              getCustomFont("Order ID: ${a.order.id}", 14, textColor, 1),
                              getCustomFont("Total: AED ${a.order.total}", 14, textColor, 1),
                              getCustomFont("Employees: ${a.order.employeeCount}", 14, textColor, 1),
                              getCustomFont("Category: ${a.category.name}", 14, textColor, 1),
                              getVerSpace(10),
                              Row(
                                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                children: [
                                  Container(
                                    ...
                                    decoration: BoxDecoration(
                                      color: _getBadgeColor(a.workStatus).withOpacity(0.2),
                                      borderRadius: BorderRadius.circular(8),
                                    ),
                                    child: getCustomFont(
                                      a.workStatus.capitalize(),
                                      ...
                                      onPressed: () async {
                                        ...
                                        final result = await showDialog<String>(
                                          ...
                                          builder: (context) => AlertDialog(
                                            ...
                                            content: TextField(
                                              controller: noteController,
                                              decoration: const InputDecoration(labelText: "Enter notes (optional)"),
                                            ),
                                            actions: [
                                        ...
                                        if (result != null) {
                                          context.read<SubscriptionScheduleBloc>().add(
                                            UpdateAssignmentStatus(a.assignmentId, "accepted", result),
                                          );
                                        }
                                      },
                              ...
                              Row(
                                children: [
                                  GestureDetector(
                                    onTap: () => launchWhatsApp(a.customer.phone),
                                    child: Container(
                                      height: 40,
                                      width: 40,
                                  ...
                                  getHorSpace(10),
                                  GestureDetector(
                                    onTap: () => launchCall(a.customer.phone),
                                    child: Container(
                                      height: 40,
                                      width: 40,
                                      decoration: BoxDecoration(
